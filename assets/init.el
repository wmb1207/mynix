...
;; Packages are managed by Nix - no need for package archives
;; Disable package.el since all packages come from NixOS
(setq package-enable-at-startup nil)
(setq package-archives nil)

;; Still load the packages list for reference, but don't install
...
;; No need to require package since packages are managed by Nix
;; (require 'package)
(require 'whitespace)
(require 'lsp-mode)
(require 'go-mode)
...
(defun theming ()
  ...
  ;; (require 'buffer-box)
  ;; (buffer-box)
  (set-mouse-color "white")  ;; White cursor
  ;; (custom-set-faces
 ...
 ;; Header-line (even though we disabled it, keeps consistency)
 (set-face-attribute 'header-line nil
		     :height my-ui-scale
		     :box nil)
...
;; (when (daemonp)
;;   (setenv "DISPLAY" ":0")
;;   ;; Create an invisible frame to initialize GUI
;;   (make-frame-invisible (make-frame)))
;;   ;; (exec-path-from-shell-initialize))
